...
function loadDataAndSync(shopId) {
    ...
    // ১. প্রথমে লোকাল স্টোরেজ থেকে ডাটা লোড করা হবে (First Render Fast)
    ...
    // ২. যদি ট্রায়াল ইউজার হয় বা অফলাইন থাকে, তাহলে ক্লাউড কল করার দরকার নেই
    if (isTrial || !db || !navigator.onLine) { 
        finalizeUIRender(); 
        return; 
    }
    
    // ৩. শুধুমাত্র প্রো/পেইড ইউজারদের ক্লাউড থেকে ডাটা সিঙ্ক করা হবে
...
function saveData() {
    ...
    // ট্রায়াল ইউজারদের ডাটা ক্লাউডে যাবে না, শুধু ফোনেই সেভ থাকবে
...
// Secret shortcut for Admin Panel Modal (Ctrl+Shift+A)
document.addEventListener('keydown', function(event) {
    if (event.ctrlKey && event.shiftKey && event.key === 'A') {
        let modal = document.getElementById('adminLicenseModal');
...
function applyGlobalBranding() {
    ...
    let landingAppName = document.getElementById('landing-app-name');
    if (landingAppName) landingAppName.innerText = appName;
    
    let sidebarAppName = document.getElementById('sidebar-app-name');
    if (sidebarAppName) sidebarAppName.innerText = appName;
    
    if (globalLogo) {
        let landingIcon = document.getElementById('landing-app-icon');
        if (landingIcon) { 
            landingIcon.src = globalLogo; 
            landingIcon.style.display = 'inline-block'; 
            let defaultIcon = document.getElementById('landing-app-icon-default');
            if (defaultIcon) defaultIcon.style.display = 'none'; 
        }
        
        let sidebarIcon = document.getElementById('sidebar-app-icon');
        if (sidebarIcon) { 
            sidebarIcon.src = globalLogo; 
            sidebarIcon.style.display = 'inline-block'; 
            let defaultIcon = document.getElementById('sidebar-app-icon-default');
            if (defaultIcon) defaultIcon.style.display = 'none'; 
        }
    }
}
...
function applyShopBranding() {
    let shopNameText = currentShopID ? (localStorage.getItem(`pos_${currentShopID}_shopname`) || "My Shop") : "My Shop"; 
    let userPic = currentShopID ? localStorage.getItem(`pos_${currentShopID}_user_pic`) : null;
    const logoTextH2 = document.getElementById('main-logo-text');
    
    if (logoTextH2) { 
        if (userPic && userPic.startsWith('data:image')) { 
            logoTextH2.innerHTML = `
            <img src="${userPic}" style="width: 30px; height: 30px; border-radius: 5px; object-fit: cover; vertical-align: middle; border: 2px solid #fff; margin-right: 5px; flex-shrink: 0;"> 
            <span id="shop-name-display" style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis; color:#fff;">${shopNameText}</span>`; 
        } else { 
            logoTextH2.innerHTML = `
            <i class="fa-solid fa-store" style="flex-shrink: 0; color:#fff;"></i> 
            <span id="shop-name-display" style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis; color:#fff;">${shopNameText}</span>`; 
        } 
    }
}
...
function loadUserProfileData() {
    let profileName = document.getElementById('profile-user-name');
    if (profileName) profileName.value = currentShopID ? (localStorage.getItem(`pos_${currentShopID}_user_name`) || "") : "";
    
    let profileShop = document.getElementById('profile-shop-name');
    if (profileShop) profileShop.value = currentShopID ? (localStorage.getItem(`pos_${currentShopID}_shopname`) || "My Shop") : "My Shop";
    
    let profilePhone = document.getElementById('profile-user-phone');
    if (profilePhone) profilePhone.value = currentShopID ? (localStorage.getItem(`pos_${currentShopID}_wanumber`) || "") : "";
    
    let profileEmail = document.getElementById('profile-user-email');
    if (profileEmail) profileEmail.value = currentShopID ? (localStorage.getItem(`pos_${currentShopID}_user_email`) || "") : "";
    
    let profileShopId = document.getElementById('profile-display-shopid');
    if (profileShopId) profileShopId.innerText = currentShopID || "Not Logged In";
    
    const savedPic = currentShopID ? localStorage.getItem(`pos_${currentShopID}_user_pic`) : null;
    let displayImg = document.getElementById('display-profile-img');
    
    if (savedPic && savedPic.startsWith('data:image')) { 
        if (displayImg) displayImg.src = savedPic; 
    } else { 
        if (displayImg) displayImg.src = "https://via.placeholder.com/120?text=User"; 
    }
}
...
function saveSettings() {
    ...
    let settings = { 
        shopAddress: document.getElementById('setting-shop-address').value, 
        currency: document.getElementById('setting-currency').value, 
        taxPercent: document.getElementById('setting-tax-percent').value, 
        invHeader: document.getElementById('setting-inv-header').value, 
        invFooter: document.getElementById('setting-inv-footer').value, 
        printSize: document.getElementById('setting-print-size').value, 
        showDue: document.getElementById('setting-show-due').checked, 
        lowStockLimit: document.getElementById('setting-low-stock-limit').value 
    };
    
    localStorage.setItem(`pos_${currentShopID}_settings`, JSON.stringify(settings)); 
    alert("✅ Settings Saved Successfully!");
}

function loadSettings() {
    let saved = localStorage.getItem(`pos_${currentShopID}_settings`);
    
    if (saved) { 
        let settings = JSON.parse(saved); 
        if (document.getElementById('setting-shop-address')) document.getElementById('setting-shop-address').value = settings.shopAddress || ""; 
        if (document.getElementById('setting-currency')) document.getElementById('setting-currency').value = settings.currency || "৳"; 
        if (document.getElementById('setting-tax-percent')) document.getElementById('setting-tax-percent').value = settings.taxPercent || "0"; 
        if (document.getElementById('setting-inv-header')) document.getElementById('setting-inv-header').value = settings.invHeader || ""; 
        if (document.getElementById('setting-inv-footer')) document.getElementById('setting-inv-footer').value = settings.invFooter || ""; 
        if (document.getElementById('setting-print-size')) document.getElementById('setting-print-size').value = settings.printSize || "thermal"; 
        if (document.getElementById('setting-show-due')) document.getElementById('setting-show-due').checked = settings.showDue; 
        if (document.getElementById('setting-low-stock-limit')) document.getElementById('setting-low-stock-limit').value = settings.lowStockLimit || "5"; 
    }
}
...
function restoreJSONBackup(event) {
    ...
    reader.onload = function(e) { 
        try { 
            ...
            if (data.customers) customersDB = data.customers; 
            if (data.products) productsDB = data.products; 
            if (data.sales) salesHistoryDB = data.sales; 
            if (data.due_collections) dueCollectionHistoryDB = data.due_collections; 
            if (data.expenses) expensesDB = data.expenses; 
...
function factoryResetApp() {
    if (confirm("⚠️ সাবধান! আপনি কি নিশ্চিত সব ডাটা মুছে ফেলতে চান? এটি আর ফেরত পাওয়া যাবে না!")) {
        let pass = prompt("নিশ্চিত করতে আপনার লগইন পাসওয়ার্ড বা 'RESET' লিখুন:");
        if (pass) { 
            customersDB = []; 
            productsDB = []; 
            salesHistoryDB = []; 
            ...
            if (db && navigator.onLine && !currentShopID.startsWith("TRIAL-")) {
                db.collection('shops').doc(currentShopID).set({ 
                    customers: [], 
                    products: [], 
                    sales: [], 
